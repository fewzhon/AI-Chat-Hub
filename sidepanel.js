...
// =====================================================================
// SITE_CONFIGS / REGION_ORDER / REGION_LABELS live in platforms.js
// (loaded by sidepanel.html before this script). We alias them here
// as locals so the rest of this file reads unchanged, and any custom
// platforms merged in at runtime via loadCustomPlatforms still mutate
// the same shared object (platforms.js attaches to window).
// =====================================================================
const SITE_CONFIGS = window.SITE_CONFIGS;
const REGION_ORDER = window.REGION_ORDER;
const REGION_LABELS = window.REGION_LABELS;

// --- The SITE_CONFIGS literal that used to live here has moved to
// --- platforms.js. To add or edit a platform, edit that file.
...
class AiChatHub {
  ...
  reloadCurrentView() {
    ...
    if (cfg.kind === 'web') {
      ...
      // Reload every panel currently in the Compare roster - the new
      // tabbar model lets the user add/remove panels at runtime, so we
      // can't iterate a static list any more. Use the live `panels`
      // array (each item carries an instanceId).
      for (const panel of (this.compareView.panels || [])) {
        this.compareView.reloadPanel(panel.instanceId);
      }
    }
  }
