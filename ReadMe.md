...
- **Customisable, persistent tab bar (starts empty)**: Always-visible scrollable tab strip at the top of the side panel. On first install the bar is **empty** so you only see the AI services you actually use - the welcome screen guides you to add your first one via the **+** button.
    ...
    -   **Custom platforms now load properly inside iframes**: adding a custom URL now requests **per-domain host permission** at the moment you click Add (Chrome shows a small prompt for that specific domain only), and registers a dynamic header-stripping rule so `X-Frame-Options` / `Content-Security-Policy` headers can't block the embed. If you deny the permission prompt the platform isn't added; you can revoke any time via `chrome://extensions` → AI Chat Hub → **Site access**. If a site still refuses to embed for unrelated reasons, the ↗ **Open in new tab** button is still there.
