...
- **Compare mode (multi-AI broadcast)**: Build a custom roster of up to **8 AI panels** drawn from the full 30-platform catalogue (or your custom platforms). A new chip-style **Compare tabbar** lets you **add (+)**, **remove (×)**, **collapse (▾/▸)**, and **drag to reorder** any panel. The same model can be added **multiple times** for prompt A/B testing — auto-named `Claude`, `Claude (2)`, `Claude (3)`, etc. AIs without broadcast-injector support get a `manual` badge and a one-click **Copy prompt** button so you can paste manually. Send All targets every **visible** (non-collapsed) panel. The full-tab **Pop out** view offers eight layouts (see below). Roster persists in `chrome.storage.local` under `compareSessionV1` and live-syncs across the side panel and popout via `chrome.storage.onChanged`.
